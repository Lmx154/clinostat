...
const Settings = ({ isOpen, onClose, onApplyPreset }) => {
  ...
  // **(1) Handler to remove a preset by its index**
  const handleDeletePreset = (index) => {
    // Filter out the preset at the given index
    setPresets((prevPresets) => 
      prevPresets.filter((_, i) => i !== index)
    );
  };
      ...
      >
        ...
        <div className="space-y-4">
          {presets.map((preset, index) => (
            <div 
              key={index} 
              className="preset-item p-2 border rounded hover:bg-gray-200 flex justify-between items-center"
            >
              {/* Clicking the left part applies preset */}
              <div 
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation(); // Stop the click from also triggering apply
